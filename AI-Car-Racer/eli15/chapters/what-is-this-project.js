...
export default {
  ...
  body: [
    ...
    'geometrically similar past tracks. In practice, measurements on this codebase show the',
    'current 6→8→4 network overfits to track-specific sensor patterns, so cross-shape transfer',
    'currently doesn\'t help (see the <code>track-similarity</code> chapter for the honest story).',
    'The bridge is still useful as save-and-resume and as a demo of the vector-memory',
    'architecture — the educational scaffolding (CNN embedder, HNSW search, GNN reranker,',
    'LoRA track adapter, lineage DAG, SONA pattern bank) is the point of the project.</p>',
